...
> 2026-09-23 기준 · 18개 학습 분야 · 학습 노트 216개 · Spring Boot 입문 노트 26개
>
> 개발 경험과 프로젝트 소개는 [GitHub 프로필](https://github.com/j-moo)에서 확인할 수 있습니다.

## Start Here

- **최신 강의노트**: [Transactional Outbox와 이벤트 발행 일관성](./SpringBoot/25_09_23_Transactional_Outbox_and_Event_Publishing/09_23_Transactional_Outbox_and_Event_Publishing.md) — 주문 DB와 outbox의 원자적 저장, lease 기반 relay·재시도, consumer inbox와 중복 전달을 상세 주석으로 설명합니다.
- **현재 학습 중**: [Spring Boot 로드맵](./SpringBoot/README.md) — 이벤트 발행의 실패 복구를 정리했고, 다음은 Saga와 보상 트랜잭션입니다.
...
| 주제 | 핵심 내용 | 노트 |
| --- | --- | --- |
| Spring Boot 입문 시리즈 | IoC·DI, MVC·REST, JDBC·JPA, 트랜잭션·테스트·보안·운영, 외부 HTTP 연동·조회 최적화·마이그레이션·동시성 제어 | [로드맵](./SpringBoot/README.md) |
...
2026년 7월 3일을 기준점으로, 9월 23일까지 **104개의 노트**를 추가했습니다. 아래는 이 기간의 학습 흐름이며 프로필 갱신 날짜와는 별도로 관리합니다.
...
| 2026.09.06 | Spring Boot | 단위·MVC·JPA·전체 구성 테스트, MockitoBean, HTTP 계약과 테스트 격리 |
| 2026.09.07 | Spring Boot | Security 필터, 인증·인가, 비밀번호 해시, HTTP Basic, CSRF와 허용·거부 테스트 |
| 2026.09.08 | Spring Boot | 로그 레벨·민감 정보, Actuator 접근·노출·권한, liveness·readiness, 메트릭과 안전한 종료 |
...
| 2026.09.13 | Spring Boot | Entity·DTO 조회 차이, 인터페이스·record Projection, SQL 관찰과 커서 계약 유지 |
| 2026.09.14 | Spring Boot | N+1 재현, 일반 join·fetch join·EntityGraph, 컬렉션 페이징과 부모 ID 기반 2단계 조회 |
| 2026.09.16 | Spring Boot | Batch Fetching 설정·Entity와 컬렉션의 묶음 단위, 후보·캐시·접근 순서와 조회 전략 비교 |
| 2026.09.17 | Spring Boot | PostgreSQL 실행 계획·복합 인덱스·탐색과 정렬 비용, 커서 경계와 결과 유지 검증 |
...
| 2026.09.23 | Spring Boot | Transactional Outbox, lease 기반 polling relay, 재시도·중복 전달·consumer inbox와 순서 정책 |
...
| [Spring Boot](./SpringBoot) | IoC·DI, MVC·REST, JDBC·JPA·트랜잭션·테스트·보안·운영, 외부 HTTP 연동·조회 최적화·마이그레이션·동시성·메시지 전달 | 26 |
| [Git](./Git) | 버전 관리, 원격 저장소와 GitHub Actions CI | 3 |
| [Markdown](./markdown) | Markdown 문법과 문서 작성 연습 | 2 |

현재 총 **216개의 학습 노트**를 관리하고 있습니다. README·작성 프롬프트·점검 보고서는 학습 노트 수에서 제외합니다.
...
2. **다음 학습 — Saga와 보상 트랜잭션**: 여러 서비스에서 단계별 commit·실패·재시도를 조정하고, 보상 작업과 재진입 가능한 상태를 설계합니다.
