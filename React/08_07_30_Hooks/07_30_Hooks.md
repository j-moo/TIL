...
Hook은 React 기능을 함수 컴포넌트에 연결하는 함수다. `useState(0)`의 반환값은 현재 상태와 setter가 들어 있는 길이 2의 배열이며, 배열 구조 분해로 각 자리에 이름을 붙인다.
...
`count`와 `setCount`는 React가 강제한 이름이 아니다. 상태의 의미가 드러나도록 개발자가 정한다. 다만 `[값, set값]` 형태를 지키면 읽는 사람이 두 역할을 즉시 구분할 수 있다.

Hook은 일반 함수처럼 보이지만 호출 위치에는 규칙이 있다. 조건문, 반복문, 중첩 함수 안에서 호출하면 렌더링마다 Hook 호출 순서가 달라질 수 있다. 따라서 컴포넌트나 커스텀 Hook의 최상위에서 항상 같은 순서로 호출한다.
