...
#### `async/await` 코드의 실행 순서 직접 읽기

`await`는 JavaScript 전체를 멈추는 명령이 아니다. 해당 `async` 함수의 실행만 잠시 양보하고, 바깥의 동기 코드는 계속 실행된다. 기다리던 Promise가 완료되면 `await` 다음 줄이 microtask로 이어진다.

```javascript
async function loadMessage() {
  console.log('2. 함수 시작')

  // 이미 이행된 Promise라도 await 뒤의 코드는 현재 동기 코드가 끝난 후 이어진다.
  const message = await Promise.resolve('데이터 준비 완료')
...
// async 함수는 호출 즉시 실행되지만, 항상 Promise를 반환한다.
const task = loadMessage()
...
1. 일반 동기 코드를 위에서 아래로 실행한다.
2. `async` 함수도 호출 즉시 시작하여 첫 `await`까지 실행한다.
3. `await`를 만나면 함수의 나머지 부분을 나중에 이어서 실행하도록 등록한다.
4. 현재 호출 스택의 동기 코드가 모두 끝난다.
5. Promise가 이행되면 `await` 다음 줄을 이어서 실행한다.

`fetch()`도 호출 즉시 Promise를 반환한다. `await fetch(url)`은 HTTP 응답 헤더가 도착할 때까지 해당 함수의 다음 줄을 미루며, `await response.json()`은 응답 본문을 읽고 JSON으로 변환할 때까지 다시 기다린다.

```javascript
async function getArticle() {
  try {
    // 첫 번째 await: 서버 응답을 기다린다.
    const response = await fetch('/api/articles/1/')

    // fetch는 404나 500에서도 Promise를 거절하지 않으므로 직접 확인한다.
    if (!response.ok) {
      throw new Error(`요청 실패: ${response.status}`)
    }

    // 두 번째 await: 응답 본문을 JSON 객체로 변환할 때까지 기다린다.
    const article = await response.json()
    return article
  } catch (error) {
    // 네트워크 오류, JSON 변환 오류, 직접 던진 HTTP 오류가 모인다.
