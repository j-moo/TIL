...
다음은 [낙관적 잠금과 동시 수정 충돌](../22_09_20_Optimistic_Locking/09_20_Optimistic_Locking.md)이다. 스키마를 일관되게 관리한 뒤에도 두 요청이 같은 행을 동시에 변경하면 한쪽 수정이 사라질 수 있다. 버전 컬럼과 트랜잭션 경계로 이러한 충돌을 감지하고 응답하는 방법으로 이어진다.
