...
현재는 비관적 행 잠금과 조건부 UPDATE로 재고 차감의 동시 요청을 제어하고, 잠금 대기·교착·timeout과 재시도 기준을 정리했다. 다음에는 멱등성 키로 같은 요청의 중복 전달을 식별한다.
...
| 24 | 멱등성 키와 중복 요청 방지 | timeout·재시도에도 주문·차감이 한 번만 반영되는 요청 계약 이해 | 예정 |
