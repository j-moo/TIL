...
현재는 offset에 이어 복합 정렬 키로 커서 기반 조회를 구성하고, 커서 검증과 삽입·삭제 시 탐색 경계를 정리했다. 다음에는 JPA DTO Projection과 조회 최적화를 학습한다.
...
| 17 | JPA DTO Projection과 조회 최적화 | 필요한 컬럼 조회·DTO 변환 방식과 정렬 키·응답 계약 유지 | 예정 |
