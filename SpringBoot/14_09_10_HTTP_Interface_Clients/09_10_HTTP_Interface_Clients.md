...
다음 확장 주제는 [페이지네이션·정렬과 조회 API](../15_09_11_Pagination_and_Sorting/09_11_Pagination_and_Sorting.md)다. 지금은 외부 검색 요청에 `page` 값을 전달했지만, 페이지 번호·크기·정렬을 서버가 어떻게 검증하고 DB 조회와 응답 DTO에 연결할지는 별도로 배워야 한다.
